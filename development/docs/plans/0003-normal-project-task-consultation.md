...
# Move Astra consultation to a normal project task

## Goal

Preserve the independent parallel Astra consultation without consuming a non-closable Codex subagent slot.
...
Blocked by: []
Decisions: [ADR-0001]
Outcome: Astra runs in a temporary fresh normal task in the current project and that task is archived after durable result capture.
...
Evidence: ["Tests: 24 adapter and task-contract tests passed", "Validation: installable Skill and native Plan profile passed", "Scope: local-only; no install commit push or release performed"]
