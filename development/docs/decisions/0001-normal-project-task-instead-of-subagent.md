...
The Astra consultation runs in a fresh normal Codex task in the current saved project. The caller preserves its result, archives the task, and temporarily unarchives the same task only for an explicit follow-up. No Codex subagent is used.
...
Keeping the subagent would retain the slot problem. Running Astra in the caller would lose independence. A second Codex CLI would add another transport and authentication boundary.
