...
TOP_LEVEL_CONFIG_KEYS = {"claude", "astra"}
CLAUDE_CONFIG_KEYS = {
    "command",
    "model",
    "effort",
...
ASTRA_CONFIG_KEYS = {"model", "effort"}
...
def load_config(path: Path) -> dict[str, Any]:
    ...
    astra = _require_object(payload["astra"], "astra", ASTRA_CONFIG_KEYS, path)

    for label, value in (("claude.model", claude["model"]), ("astra.model", astra["model"])):
        try:
            model_name(value)
        except (argparse.ArgumentTypeError, TypeError) as error:
            raise ValueError(f"invalid {label} in config {path}: {error}") from error
    for label, value in (("claude.effort", claude["effort"]), ("astra.effort", astra["effort"])):
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "The calling Codex owns the paired normal Astra project task."
        )
    )
    parser.add_argument(
        "--config",
