...
model/effort. If it cannot, continue with Claude and report Astra as
...
1. Create Astra first without waiting for its answer:
   ...
   - request `gpt-6-astra` and `xhigh` unless overridden in the actual creation
     call;
...
4. Let both continue concurrently, then collect Astra through the host's normal
   task wait/read controls. Do not wait for Astra before starting Claude.
...
2. Unarchive the existing Astra task, send the follow-up with the host's task
   messaging control, and do not wait before starting Claude.
3. Immediately pipe it to the Claude adapter with `--resume
   <claude-session-id>`.
4. Collect and attribute both results, preserve Astra's answer, then archive its
   task again.

For an Astra-only follow-up, unarchive and message the retained Astra task,
collect and preserve the answer, then archive it again. For a Claude-only
...
provider and report a partial result. Never call a newly created Astra task a
continuation of a missing task ID.

Do not keep Astra unarchived for a hypothetical follow-up. Archiving preserves
the task ID and context; a later explicit follow-up may unarchive and continue it.
